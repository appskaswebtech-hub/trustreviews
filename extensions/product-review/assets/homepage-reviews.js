(function () {
  ...
  function buildTicker(reviews, s) {
    ...
    for (var pass = 0; pass < 2; pass++) {
      for (var i = 0; i < items.length; i++) {
        ...
        item.innerHTML =
          '<span class="tr-extensions-product-review-assets-homepage-reviews-span-6">' + starHTML(r.rating, s.starColor) + '</span>' +
          '<span class="hr-ticker-name">' + (r.customer || 'Customer') + '</span>' +
          '<span class="tr-extensions-product-review-assets-homepage-reviews-span-7">' + (r.comment || '').slice(0, 60) + (r.comment && r.comment.length > 60 ? '…' : '') + '</span>';
  ...
  function buildSplitScreen(reviews, s, t) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      row.innerHTML =
        ...
        '<strong class="tr-extensions-product-review-assets-homepage-reviews-strong-9">' + (r.customer || 'Customer') + '</strong></div>';
  ...
  function buildStackedCards(reviews, s, t) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      card.innerHTML = '<div class="tr-extensions-product-review-assets-homepage-reviews-div-13">' + starHTML(r.rating, s.starColor) + '</div>' +
        '<p class="hr-stacked-text">' + (r.comment || '') + '</p><strong class="tr-extensions-product-review-assets-homepage-reviews-strong-14">' + (r.customer || 'Customer') + '</strong>';
  ...
  function buildAccordionPanels(reviews, s, t) {
    ...
    for (var i = 0; i < items.length; i++) {
      (function (r, isFirst) {
        ...
        head.innerHTML = '<span class="tr-extensions-product-review-assets-homepage-reviews-span-15">' + starHTML(r.rating, s.starColor) + '</span><strong class="tr-extensions-product-review-assets-homepage-reviews-strong-16">' + (r.customer || 'Customer') + '</strong><span class="hr-acc-arrow">&#9662;</span>';
        var body = document.createElement('div');
        body.className = 'hr-acc-body';
        body.innerHTML = '<p class="tr-extensions-product-review-assets-homepage-reviews-p-17">' + (r.comment || '') + '</p>';
  ...
  function initWidget(el) {
    ...
    Promise.all([
      ...
      .then(function (results) {
        ...
        reviews.forEach(function (r) { r.mediaUrl = trResolveMedia(r.mediaUrl); });
