...
function OnboardingBounce({ url }) {
  ...
  return (
    ...
    }} className="tr-app-routes-app-div-1">
      <div style={{
        background: "#fff", border: "1px solid #e5e4ec", borderRadius: 14,
        padding: "32px 28px", maxWidth: 380, textAlign: "center",
      }} className="tr-app-routes-app-div-2">
        <div style={{ fontSize: 17, fontWeight: 600, color: "#17171c", marginBottom: 8 }} className="tr-app-routes-app-div-3">
          Let&apos;s get you set up
        </div>
        <p style={{ fontSize: 13.5, color: "#6b6b78", lineHeight: 1.6, marginBottom: 20 }} className="tr-app-routes-app-p-4">
        ...
        <a
         ...
         className="tr-app-routes-app-a-5">
          Continue →
        </a>
      </div>
    </div>
...
// None of the embedded onboarding popup's submissions to
// /app/onboarding-embed should trigger this loader to revalidate:
//  - saveProgress (fired on every Continue/Skip, see
//    EmbeddedOnboardingModal's advance()) persists onboardingStep in the
//    background — a revalidation here would hit the loader's "already
...
//    plan.
//  - advance (the Plan step's "Try Advanced" button) hits that exact same
//    fallback for a different reason: saveProgress already set
//    onboardingStep to 3 the moment the Plan step was reached, so a
//    revalidation triggered by advance's own action sees onboardingStep >= 3
//    and finalizes as Free right as the popup is trying to break out to
//    Shopify's billing confirmation page — the same race that broke the
//    standalone onboarding page (see routes/onboarding.jsx's shouldRevalidate).
export function shouldRevalidate({ formAction, formData, defaultShouldRevalidate }) {
  if (formAction === "/app/onboarding-embed") {
    const actionType = formData?.get("actionType");
    if (actionType === "saveProgress" || actionType === "advance") return false;
  }
