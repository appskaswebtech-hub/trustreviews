...
// ── "Auto" Saved widget resolution ─────────────────────────────────────────────
// Shopify's addAppBlockId deep link can't prefill block settings, so every
// widget in the admin gallery installs the same "Trust Reviews" block. A block
// left on "Auto" asks here which widget it is:
//   • already assigned            → that widget, forever
//   • new block in the Theme Editor, and a widget's "Install to Theme" was
//     clicked in the last 30 min and not yet claimed → that widget
//   • anything else               → review_widget (pinned, so an existing
//     block can never grab a later install)
// Storefront (non-editor) renders of an unassigned block pin to the default
// too, so blocks already live on the store are locked before any install.
const AUTO_WIDGET_KEYS = new Set([
  "review_widget", "cards_carousel", "single_review", "testimonial_carousel", "videos_carousel",
  "popup_reviews", "reviews_grid", "happy_customers", "review_snippets", "summary_list",
  "classic_list", "floating_reviews_tab", "insta_stories", "hero_quote_carousel",
  "coverflow_carousel", "split_media_carousel",
]);
const AUTO_CLAIM_WINDOW_MS = 30 * 60 * 1000;

async function resolveAutoWidgetKey(shop, blockId, designMode) {
  if (!blockId) return "review_widget";
  try {
    const existing = await prisma.widgetBlockAssignment.findUnique({
      where: { shop_blockId: { shop, blockId } },
      select: { widgetKey: true },
    });
    if (existing) return existing.widgetKey;

    let widgetKey = "review_widget";
    if (designMode) {
      const install = await prisma.widgetInstall.findFirst({
        where: {
          shop,
          widgetKey: { in: [...AUTO_WIDGET_KEYS] },
          installedAt: { gte: new Date(Date.now() - AUTO_CLAIM_WINDOW_MS) },
        },
        orderBy: { installedAt: "desc" },
        select: { widgetKey: true, installedAt: true },
      });
      // Unclaimed = no block has taken *this* widget since the click. Default
      // pins of other blocks (e.g. a visitor's first render) don't consume it.
      const claimed = install && await prisma.widgetBlockAssignment.findFirst({
        where: { shop, widgetKey: install.widgetKey, createdAt: { gte: install.installedAt } },
        select: { id: true },
      });
      if (install && !claimed) widgetKey = install.widgetKey;
    }

    // upsert with an empty update: if two renders race, the first write wins
    // and both end up reading the same assignment.
    const saved = await prisma.widgetBlockAssignment.upsert({
      where: { shop_blockId: { shop, blockId } },
      update: {},
      create: { shop, blockId, widgetKey },
      select: { widgetKey: true },
    });
    return saved.widgetKey;
  } catch (e) {
    // Table not migrated yet (run `npx prisma db push`) — behave like before.
    console.error("[auto widget] resolve failed:", e.message);
...
export async function loader({ request }) {
  ...
  if (url.searchParams.get("type") === "widget-defaults") {
    let widgetKey = url.searchParams.get("widgetKey") || "review_widget";
    if (widgetKey === "auto") {
      widgetKey = await resolveAutoWidgetKey(
        shop,
        url.searchParams.get("blockId"),
        url.searchParams.get("designMode") === "true",
      );
    }
    ...
    // If the saved layout is Advanced-plan-only and the shop isn't currently
