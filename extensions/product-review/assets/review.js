...
function buildCustomCardHTML(r, template, q) {
  ...
  var titleBlock = r.title
    ...
    : "";
  var reply = r.reply
    ? '<div class="tr-reply"><strong class="tr-extensions-product-review-assets-review-strong-2">Store reply:</strong> ' + escapeHTML(r.reply) + '</div>'
    : "";
  var media = "";
  if (r.mediaUrl) {
    ...
    if (r.mediaType && r.mediaType.startsWith("image/")) {
      media = '<div class="tr-media"><img class="tr-extensions-product-review-assets-review-img-3" src="' + mUrl + '" alt="Review image"></div>';
    } else if (r.mediaType && r.mediaType.startsWith("video/")) {
      media = '<div class="tr-media"><video class="tr-extensions-product-review-assets-review-video-4" src="' + mUrl + '" controls></video></div>';
    }
  }
  return template
    .replace(/\{\{id\}\}/g,         r.id)
...
function buildCardHTML(r, q, style){
  var stars = "";
  for(var i = 1; i <= 5; i++){
    stars += i <= r.rating ? '<span class="tr-extensions-product-review-assets-review-span-5">★</span>' : '<span class="dim">★</span>';
  }
  var initials     = getInitials(r.customer);
  ...
  try {
    ...
    for(var i = 1; i <= 5; i++){
      avgStars += i <= Math.round(avgValue)
        ? '<span class="tr-extensions-product-review-assets-review-span-6">★</span>'
        : '<span class="tr-extensions-product-review-assets-review-span-7" style="color:#ddd">★</span>';
    }
