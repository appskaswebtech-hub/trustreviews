  ...
  document.querySelectorAll(".rw-widget").forEach(function (widget) {
    ...
    function buildCard(r, q) {
      ...
      var replyHTML = r.reply
        ? '<div class="rw-card__reply"><strong class="tr-extensions-product-review-assets-review-wall-strong-1">' + (T.storeReplyLabel || 'Store reply') + ':</strong> ' + escapeHTML(r.reply) + "</div>"
        : "";
      var mediaHTML = "";
      if (r.mediaUrl && r.mediaType && r.mediaType.startsWith("image/")) {
        mediaHTML = '<div class="rw-card__media"><img class="tr-extensions-product-review-assets-review-wall-img-2" src="' + escapeHTML(resolveMedia(r.mediaUrl)) + '" alt="Review image" loading="lazy"></div>';
      }
      return (
        '<div class="rw-card">' +
    ...
    function renderPagination(totalPages, total) {
      ...
      pages.forEach(function (p) {
        if (p === "…") {
          html += '<span class="tr-extensions-product-review-assets-review-wall-span-3" style="padding:0 4px;color:#9ca3af">…</span>';
        } else {
    ...
    async function loadWall() {
      try {
        ...
        loadingEl.innerHTML = '<span class="tr-extensions-product-review-assets-review-wall-span-4">Could not load reviews.</span>';
      }
    }
